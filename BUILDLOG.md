...
**NEXT: P4 soul** — (1) photo mode (free cam + tilt-shift slider); (2) remaining easter eggs (§20: Konami retro, fridge poetry magnets, sunflower hum, red balloon, mirror Murray, dev room, idle campfire, credits level — pick high-charm/low-cost first); (3) secret levels (Crumb Dimension, Dev Room, Impossible Room weekly); (4) accessibility suite (§20.15 arachnophobia mode = spiders→googly-roombas swap in critterModels, colorblind patterns, shake/flash sliders exist?, UI scale, remap); (5) loss vignettes (condemned/betrayal/exterminated) + EXTERMINATOR alliance finale set-piece (the story payoff — GAME-PROMPT §8.9); (6) Weird Closet events + Garage Sale haggle (§15 leftovers, logged in CUTS). Then P5 multiplayer (§17, WebRTC lockstep — the deterministic sim was built for this from day one). Router discipline unchanged.
