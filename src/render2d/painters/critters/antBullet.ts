...
import { ramp, celCrescent, celCrescentPath } from '../../paint';
...
registerCritterPainter('ant-bullet', (ctx, size, frame, opts) => {
  ...
  const r3 = ramp(base);
  const head = warm(mix(PAL.antBullet, r3.shadow, 0.18));
  ...
  // V2: cel shadow along the dart's away-from-light flank (speed = sleek, so a
  // directional streak shadow rather than a round lens) + segment rings in the
  // hue-shifted shadow tone.
  celCrescentPath(ctx, () => {
    ctx.moveTo(cx, cy + size * 0.43);
    ctx.quadraticCurveTo(cx - size * 0.16, cy + size * 0.25, cx - size * 0.09, cy + size * 0.08);
    ctx.quadraticCurveTo(cx, cy - size * 0.01, cx + size * 0.09, cy + size * 0.08);
    ctx.quadraticCurveTo(cx + size * 0.16, cy + size * 0.25, cx, cy + size * 0.43);
    ctx.closePath();
  }, cx, cy + size * 0.22, size * 0.18, size * 0.24, r3.shadow, 0.5, 0.5);
  ctx.strokeStyle = rgba(r3.shadow, 0.8); ctx.lineWidth = size * 0.024;
  ...
  celCrescent(ctx, cx, thoraxY, size * 0.075, size * 0.095, r3.shadow, 0.5, 0.55);
  oval(cx, headY, size * 0.105, size * 0.115, head);
  celCrescent(ctx, cx, headY, size * 0.105, size * 0.115, r3.shadow, 0.42, 0.5);
  // sleek toward-light speed highlight along the west flank
  ctx.beginPath(); ctx.ellipse(cx - size * 0.03, cy + size * 0.19, size * 0.04, size * 0.12, -0.15, 0, Math.PI * 2); ctx.fillStyle = rgba(r3.light, 0.5); ctx.fill();
