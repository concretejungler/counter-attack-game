...
registerCritterPainter('bedbug', (ctx, size, frame, opts) => {
  ...
  const rim = warm(darken(base, 0.12));
  ...
  ctx.beginPath(); ctx.ellipse(cx, cy - size * 0.1, size * 0.17, size * 0.11, 0, Math.PI, Math.PI * 2); ctx.lineTo(cx + size * 0.17, cy - size * 0.05); ctx.quadraticCurveTo(cx, cy + size * 0.01, cx - size * 0.17, cy - size * 0.05); ctx.closePath(); ctx.fillStyle = hex(rim); ctx.fill(); stroke(size * 0.025);
  ctx.strokeStyle = rgba(darken(base, 0.28), 0.85); ctx.lineWidth = size * 0.022;
  ...
  ctx.beginPath(); ctx.ellipse(cx - size * 0.055, cy - size * 0.02, size * 0.055, size * 0.17, -0.15, 0, Math.PI * 2); ctx.fillStyle = rgba(lighten(base, 0.35), 0.36); ctx.fill();
