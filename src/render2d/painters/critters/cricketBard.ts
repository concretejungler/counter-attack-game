...
registerCritterPainter('cricket-bard', (ctx, size, frame, opts) => {
  ...
  const head = warm(mix(base, r3.shadow, 0.2));
  ...
  // V2: belly + cel on the abdomen (dominant mass); the wing plate is a hard-cel
  // sub-form, its stridulation veins toned from the ramp shadow.
  oval(cx, cy + size * 0.12, size * 0.14, size * 0.2, base);
  bellyGrad(ctx, cx, cy + size * 0.12, size * 0.13, size * 0.19, r3, 0.5);
  celCrescent(ctx, cx, cy + size * 0.12, size * 0.14, size * 0.2, r3.shadow, 0.45, 0.5);
  ...
  ctx.strokeStyle = rgba(wingR.shadow, 0.6); ctx.lineWidth = size * 0.012; for (const dx of [-0.04, 0, 0.04]) { ctx.beginPath(); ctx.moveTo(cx, cy + size * 0.01); ctx.lineTo(cx + size * dx, cy + size * 0.21); ctx.stroke(); }
  oval(cx, cy - size * 0.17, size * 0.13, size * 0.12, head);
  celCrescent(ctx, cx, cy - size * 0.17, size * 0.13, size * 0.12, r3.shadow, 0.42, 0.5);
