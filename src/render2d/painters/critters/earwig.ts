...
registerCritterPainter('earwig', (ctx, size, frame, opts) => {
  ...
  const r3 = ramp(base);
  const head = warm(mix(base, r3.shadow, 0.25));
  ...
  // V2: belly + cel on the long body; segment rings toned from the ramp shadow.
  ctx.beginPath(); ctx.ellipse(cx, cy + size * 0.08, size * 0.12, size * 0.28, 0, 0, Math.PI * 2); ctx.fillStyle = hex(base); ctx.fill(); stroke();
  bellyGrad(ctx, cx, cy + size * 0.08, size * 0.115, size * 0.27, r3, 0.5);
  celCrescent(ctx, cx, cy + size * 0.08, size * 0.12, size * 0.28, r3.shadow, 0.45, 0.5);
  ctx.strokeStyle = rgba(r3.shadow, 0.8); ctx.lineWidth = size * 0.02; for (const dy of [-0.08, 0, 0.08, 0.16]) { ctx.beginPath(); ctx.ellipse(cx, cy + size * dy, size * 0.1, size * 0.016, 0, 0, Math.PI * 2); ctx.stroke(); }
  oval(cx, cy - size * 0.22, size * 0.12, size * 0.105, head);
  celCrescent(ctx, cx, cy - size * 0.22, size * 0.12, size * 0.105, r3.shadow, 0.42, 0.5);
