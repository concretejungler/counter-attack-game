...
registerCritterPainter('maggot', (ctx, size, frame, opts) => {
  ...
  // V2 creases: ramp shadow warmed toward dusty-rose so the pale rice-grain
  // larva keeps readable, species-true (soft/pink) creases on light floors.
  const crease = mix(r3.shadow, PAL.cakeFrosting, 0.3);
  ...
  // V2: belly + cel volume on the pale bean; ramp-warmed creases keep segments.
  ctx.beginPath(); ctx.ellipse(cx, cy + size * 0.07, size * 0.19, size * 0.31, 0, 0, Math.PI * 2); ctx.fillStyle = hex(body); ctx.fill(); stroke();
  bellyGrad(ctx, cx, cy + size * 0.07, size * 0.18, size * 0.3, r3, 0.55);
  celCrescent(ctx, cx, cy + size * 0.07, size * 0.19, size * 0.31, r3.shadow, 0.45, 0.5);
  ...
  celCrescent(ctx, cx, cy - size * 0.24, size * 0.14, size * 0.11, ramp(head).shadow, 0.42, 0.5);
