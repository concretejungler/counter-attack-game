...
registerCritterPainter('mosquito', (ctx, size, frame, opts) => {
  ...
  const abdomenCol = warm(lighten(body, 0.12));
  ...
  ctx.lineCap = 'round'; ctx.strokeStyle = hex(leg); ctx.lineWidth = size * 0.016;
  ...
  // V2: flier — NO baked shadow. ONE airy belly on the abdomen; ramp-toned rings.
  ctx.beginPath(); ctx.ellipse(cx, cy + size * 0.13, size * 0.09, size * 0.2, 0, 0, Math.PI * 2); ctx.fillStyle = hex(abdomenCol); ctx.fill(); stroke();
  bellyGrad(ctx, cx, cy + size * 0.13, size * 0.085, size * 0.19, abdomenR, 0.55);
  ctx.strokeStyle = rgba(abdomenR.shadow, 0.7); ctx.lineWidth = size * 0.018; for (const dy of [0.04, 0.13, 0.22]) { ctx.beginPath(); ctx.ellipse(cx, cy + size * dy, size * 0.075, size * 0.014, 0, 0, Math.PI * 2); ctx.stroke(); }
  ctx.beginPath(); ctx.ellipse(cx, thoraxY, size * 0.095, size * 0.105, 0, 0, Math.PI * 2); ctx.fillStyle = hex(body); ctx.fill(); stroke();
  ctx.beginPath(); ctx.ellipse(cx - size * 0.03, thoraxY - size * 0.025, size * 0.04, size * 0.038, 0, 0, Math.PI * 2); ctx.fillStyle = rgba(bodyR.light, 0.5); ctx.fill();
