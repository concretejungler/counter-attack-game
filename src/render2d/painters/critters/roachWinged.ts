 ...
 * (V2 elite shading.) It keeps the roach oval and antennae, but the unfolded
 * translucent wings become the dominant shape; no baked shadow. V2: waxy chitin
 * dome (belly + cel + rim + one gloss dot), the pale wings are the cool pop.
 */
import { registerCritterPainter } from '../../spriteCache';
import { PAL } from '../../../render/palette';
import { COCOA_CSS, hex, rgba, mix, lighten } from '../../colors';
import { ramp, celCrescent, belly, rim, glossDot, innerInk } from '../../paint';
...
registerCritterPainter('roach-winged', (ctx, size, frame, opts) => {
  ...
  // --- unfolded wings (dominant): NO baked shadow (flier) ---
  for (const sgn of [-1, 1]) {
    ...
    ctx.beginPath(); ctx.ellipse(-size * 0.02, -size * 0.02, size * 0.09, size * 0.045, 0, 0, Math.PI * 2); ctx.fillStyle = 'rgba(210,235,245,0.4)'; ctx.fill();
  ...
  // --- legs ---
  ctx.strokeStyle = innerInk(body); ctx.fillStyle = innerInk(body); ctx.lineWidth = size * 0.033;
  ...
  // --- body dome (dominant mass): belly + cel + rim + gloss ---
  ctx.beginPath(); ctx.ellipse(cx, cy + size * 0.1, size * 0.18, size * 0.31, 0, 0, Math.PI * 2); ctx.fillStyle = hex(body); ctx.fill(); stroke();
  belly(ctx, cx, cy + size * 0.1, size * 0.17, size * 0.3, bodyR, 0.55);
  ctx.beginPath(); ctx.ellipse(cx, cy + size * 0.04, size * 0.12, size * 0.23, 0, 0, Math.PI * 2); ctx.fillStyle = rgba(shell, 0.5); ctx.fill();
  ctx.strokeStyle = rgba(bodyR.shadow, 0.8); ctx.lineWidth = size * 0.022; ctx.beginPath(); ctx.moveTo(cx, cy - size * 0.16); ctx.lineTo(cx, cy + size * 0.34); ctx.stroke();
  glossDot(ctx, cx - size * 0.08, cy - size * 0.06, size * 0.03, 0.6);
  celCrescent(ctx, cx, cy + size * 0.1, size * 0.18, size * 0.31, bodyR.shadow, 0.45, 0.5);
  rim(ctx, cx, cy + size * 0.1, size * 0.18, size * 0.31, bodyR.light, size * 0.022, 0.5);

  // --- head plate ---
  ctx.beginPath(); ctx.ellipse(cx, cy - size * 0.22, size * 0.13, size * 0.11, 0, 0, Math.PI * 2); ctx.fillStyle = hex(mix(body, bodyR.shadow, 0.25)); ctx.fill(); stroke();
  celCrescent(ctx, cx, cy - size * 0.22, size * 0.13, size * 0.11, bodyR.shadow, 0.42, 0.45);
