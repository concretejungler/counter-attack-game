...
registerCritterPainter('slug', (ctx, size, frame, opts) => {
  ...
  const belly = warm(lighten(PAL.slug, 0.16));
  ...
  ctx.strokeStyle = hex(darken(body, 0.28));
  ...
  stroke();
  ctx.beginPath();
  ctx.ellipse(cx - size * 0.06, cy + size * 0.04, size * 0.085, size * 0.25, -0.12, 0, Math.PI * 2);
  ctx.fillStyle = rgba(lighten(body, 0.34), 0.55);
  ctx.fill();
  ctx.beginPath();
  ctx.ellipse(cx, cy + size * 0.22, size * 0.14, size * 0.055, 0, 0, Math.PI * 2);
  ctx.fillStyle = rgba(belly, 0.72);
  ctx.fill();
