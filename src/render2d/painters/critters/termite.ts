...
registerCritterPainter('termite', (ctx, size, frame, opts) => {
  ...
  const headR = ramp(head);
  const leg = mix(headR.shadow, 0x000000, 0.1);
  ...
  // V2: belly + cel on the pale abdomen; amber head gets its own cel volume.
  ctx.beginPath(); ctx.ellipse(cx, cy + size * 0.14, size * 0.17, size * 0.24, 0, 0, Math.PI * 2); ctx.fillStyle = hex(body); ctx.fill(); stroke();
  bellyGrad(ctx, cx, cy + size * 0.14, size * 0.16, size * 0.23, r3, 0.55);
  celCrescent(ctx, cx, cy + size * 0.14, size * 0.17, size * 0.24, r3.shadow, 0.45, 0.5);
  ctx.strokeStyle = rgba(r3.shadow, 0.74); ctx.lineWidth = size * 0.022; for (const dy of [0, 0.1, 0.2, 0.29]) { ctx.beginPath(); ctx.ellipse(cx, cy + size * dy, size * 0.145, size * 0.018, 0, 0, Math.PI * 2); ctx.stroke(); }
  ctx.beginPath(); ctx.ellipse(cx, cy - size * 0.18, size * 0.16, size * 0.14, 0, 0, Math.PI * 2); ctx.fillStyle = hex(head); ctx.fill(); stroke();
  celCrescent(ctx, cx, cy - size * 0.18, size * 0.16, size * 0.14, headR.shadow, 0.42, 0.5);
