/**
 * PAINTER - Stag Buddy (id 'jar-stag'). Mason jar blocker tank.
...
registerTowerPainter('jar-stag', (ctx, size, _frame, opts) => {
  ...
  for (const sx of [-1, 1]) {
    ...
    ctx.quadraticCurveTo(cx + sx * size * 0.2, y + h * 0.14, cx + sx * size * 0.17, y + h * 0.39);
    ctx.strokeStyle = hex(accent);
    ctx.lineWidth = size * 0.035;
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(cx + sx * size * 0.13, y + h * 0.25);
    ctx.lineTo(cx + sx * size * 0.2, y + h * 0.2);
    ctx.strokeStyle = COCOA_CSS;
    ctx.lineWidth = size * 0.018;
    ctx.stroke();
  }
  ctx.beginPath();
  ctx.ellipse(cx, y + h * 0.47, size * 0.14, size * 0.18, 0, 0, Math.PI * 2);
