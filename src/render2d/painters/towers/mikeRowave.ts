/**
 * PAINTER - Mike Rowave (id 'mike-rowave'). break-room microwave blaster.
...
registerTowerPainter('mike-rowave', (ctx, size, _frame, opts) => {
  ...
  ctx.fillStyle = hex(PAL.metalDark);
  ctx.fill();
  drawFace(cx - size * 0.05, y + h * 0.38, size * 0.075, size * 0.038, -0.05, 'smirk');
  drawPips(size * 0.86);
  ascendRect(x - ink, y - ink * 0.5, w + ink * 2, h + ink, size * 0.1, y + h * 0.48);
