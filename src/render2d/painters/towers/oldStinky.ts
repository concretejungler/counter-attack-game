/**
 * PAINTER - Old Stinky (id 'old-stinky'). forgotten sock gas aura.
...
registerTowerPainter('old-stinky', (ctx, size, _frame, opts) => {
  ...
  ctx.fill();
  drawFace(cx, size * 0.5, size * 0.08, size * 0.039, -0.35, 'frown');
  drawPips(size * 0.86);
  ascendRect(cx - size * 0.22, size * 0.15, size * 0.56, size * 0.67, size * 0.14, size * 0.49);
