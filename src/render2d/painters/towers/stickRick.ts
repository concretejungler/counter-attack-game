/**
 * PAINTER - Stick Rick (id 'stick-rick'). tape dispenser glue zone.
...
registerTowerPainter('stick-rick', (ctx, size, _frame, opts) => {
  ...
  const x = cx - size * 0.3, y = size * 0.37, w = size * 0.6, h = size * 0.32;
  roundRect(x, y, w, h, size * 0.12);
  ctx.fillStyle = hex(warm(PAL.metalDark));
  ctx.fill();
  strokeInk();
  ctx.beginPath();
  ctx.arc(cx - size * 0.13, y + size * 0.02, size * 0.16, 0, Math.PI * 2);
  ctx.fillStyle = rgba(lighten(accent, 0.2), 0.78);
  ctx.fill();
  strokeInk(size * 0.03);
  ctx.beginPath();
  ctx.arc(cx - size * 0.13, y + size * 0.02, size * 0.07, 0, Math.PI * 2);
  ctx.fillStyle = rgba(0xffffff, 0.55);
  ctx.fill();
  strokeInk(size * 0.017);
  ctx.beginPath();
  ctx.moveTo(cx - size * 0.01, y + size * 0.01);
  ctx.quadraticCurveTo(cx + size * 0.15, y + size * 0.06, cx + size * 0.22, y + size * 0.17);
  ctx.strokeStyle = rgba(lighten(accent, 0.25), 0.85);
  ctx.lineWidth = size * 0.045;
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(x + w - size * 0.04, y + size * 0.1);
  ctx.lineTo(x + w + size * 0.11, y + size * 0.1);
  ctx.lineTo(x + w + size * 0.04, y + size * 0.19);
  ctx.closePath();
  ctx.fillStyle = hex(PAL.metal);
  ctx.fill();
  strokeInk(size * 0.022);
  drawFace(cx - size * 0.02, y + h * 0.57, size * 0.08, size * 0.038, 0.2, 'smile');
  drawPips(size * 0.86);
  ascendRect(x - ink, size * 0.19, w + size * 0.18 + ink, h + size * 0.18, size * 0.14, y + h * 0.45);
