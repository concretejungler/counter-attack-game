/**
 * PAINTER - Vroomba (id 'vroomba'). robot vacuum sweeper.
...
registerTowerPainter('vroomba', (ctx, size, _frame, opts) => {
  ...
  strokeInk(size * 0.02);
  ctx.beginPath();
  ctx.arc(cx, cy - size * 0.1, size * 0.045, 0, Math.PI * 2);
  ctx.fillStyle = hex(accent);
  ctx.fill();
  strokeInk(size * 0.015);
  for (const sx of [-1, 1]) {
    ctx.beginPath();
    ctx.ellipse(cx + sx * size * 0.24, cy + size * 0.14, size * 0.05, size * 0.025, 0, 0, Math.PI * 2);
    ctx.fillStyle = hex(PAL.metalDark);
    ctx.fill();
  ...
  if (ascended) {
    ctx.beginPath();
    ctx.ellipse(cx, cy, rx + ink, ry + ink, 0, 0, Math.PI * 2);
    ctx.strokeStyle = hex(PAL.butter);
    ctx.lineWidth = size * 0.03;
    ctx.stroke();
    drawSparkles(cy);
